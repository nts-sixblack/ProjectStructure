...
public extension FlowPath {
    ...
    /// A computed property that converts the current routes into a `CodableRepresentation`, if possible.
    ///
    /// This property attempts to transform all routes into a codable format by ensuring their screens conform to `Codable`.
    /// If any screen is not `Codable`, the conversion fails, and `nil` is returned.
    ...
    /// Initializes a new instance from a `CodableRepresentation`.
